...
def backtest_set_up(API_KEY, API_SECRET, crypto_name = "ETH", stock_name = "BILI", ):
...
def get_stock_df(alpaca, stock_name = "BILI", timeframe = TimeFrame.Day, start_date = "2021-01-01", end_date = "2021-10-20"):
    stock_data = alpaca.get_bars(stock_name, timeframe, start_date, end_date).df
    return stock_data
def get_crtpyo_df(alpaca, crypto_name = "ETH", timeframe = TimeFrame.Day, start_date = "2021-01-01", end_date = "2021-10-20"):
    crypto_name = crypto_name+"USD"
    crypto_data = alpaca.get_crypto_bars(crypto_name, timeframe, start_date, end_date).df
    return crypto_data

#Comparing stock & crypto currency
#default computing the 5-day SMA and 13-day SMA, and 252 trading days in the current year
def CS_charts_n_sharp(stock_df ,crypto_df, stock_name, crypto_name, SMA_fast_period = 5, SMA_slow_period = 13,trading_days = 252):
    ...
    cumulative_return_fig = px.line(data,x=data.index, y=[stock_name+'_return', crypto_name+'_return'])
    ...
    orders = pd.concat([crossover[[crypto_name, 'order']], crossunder[[crypto_name,'order']]]).sort_index()
    ...
    portfolio[stock_name+'_buy_&_hold'] = (portfolio[stock_name+'_return'] + 1) * 10000
    portfolio[crypto_name+'_buy_&_hold'] = (portfolio[crypto_name+'_return'] + 1) * 10000
    ...
    portfolio[[crypto_name+'_buy_&_hold', stock_name+'_buy_&_hold', crypto_name+'_daily_return',]] = \
                        portfolio[[crypto_name+'_buy_&_hold', stock_name+'_buy_&_hold', crypto_name+'_daily_return']].ffill()
    ...
    for index, row in portfolio.iterrows():
        ...
        if active_position:
            portfolio.loc[index, crypto_name+'_SMA_crossover'] = (row[crypto_name+'_daily_return'] + 1) * equity
            equity = portfolio.loc[index, crypto_name+'_SMA_crossover']
        else:
            portfolio.loc[index, crypto_name+'_SMA_crossover'] = equity
    strategy_performance_fig=px.line(portfolio[[crypto_name+'_SMA_crossover', crypto_name+'_buy_&_hold', stock_name+'_buy_&_hold']], color_discrete_sequence=['green','blue', 'red'])
    portfolio[crypto_name+'_SMA_daily_returns'] = portfolio[crypto_name+'_SMA_crossover'].pct_change()
    ...
    daily_sharpe_ratio = (mean_daily_return - stock_mean_daily_return) / std_daily_return

    annualized_sharpe_ratio = daily_sharpe_ratio * (trading_days ** 0.5)
    # return annualized_sharpe_ratio
    return annualized_sharpe_ratio, cumulative_return_fig, SMA_fig, SMA_Crossovers_fig, strategy_performance_fig

#Comparing stock & stock
def SS_charts_n_sharp(stock1_df ,stock2_df, stock1_name, stock2_name, SMA_fast_period = 5, SMA_slow_period = 13,trading_days = 252):
    ...
    data[stock1_name+'_return'] = data[stock1_name+'_daily_return'].add(1).cumprod().sub(1)
    data[stock2_name+'_return'] = data[stock2_name+'_daily_return'].add(1).cumprod().sub(1)

    cumulative_return_fig = px.line(data,x=data.index, y=[stock1_name+'_return', stock2_name+'_return'])
    ...
    # Combine buys and sells into 1 data frame
    orders = pd.concat([crossover[[stock2_name, 'order']], crossunder[[stock2_name,'order']]]).sort_index()

    # new dataframe with market data and orders merged
    portfolio = pd.merge(data, orders, how='outer', left_index=True, right_index=True)
    # "backtest" of our buy and hold strategies
    portfolio[stock1_name+'_buy_&_hold'] = (portfolio[stock1_name+'_return'] + 1) * 10000
    portfolio[stock2_name+'_buy_&_hold'] = (portfolio[stock2_name+'_return'] + 1) * 10000
    # forward fill any missing data points in our buy & hold strategies 
    # and forward fill crypto_daily_return for missing data points
    portfolio[[stock2_name+'_buy_&_hold', stock1_name+'_buy_&_hold', stock2_name+'_daily_return',]] = \
                        portfolio[[stock2_name+'_buy_&_hold', stock1_name+'_buy_&_hold', stock2_name+'_daily_return']].ffill()
    ### Backtest of SMA crossover strategy
    active_position = False
    equity = 10000
    ...
    for index, row in portfolio.iterrows():
        ...
        # update strategy equity
        if active_position:
            portfolio.loc[index, stock2_name+'_SMA_crossover'] = (row[stock2_name+'_daily_return'] + 1) * equity
            equity = portfolio.loc[index, stock2_name+'_SMA_crossover']
        else:
            portfolio.loc[index, stock2_name+'_SMA_crossover'] = equity
    strategy_performance_fig=px.line(portfolio[[stock2_name+'_SMA_crossover', stock2_name+'_buy_&_hold', stock_name+'_buy_&_hold']], color_discrete_sequence=['green','blue', 'red'])
    portfolio[stock2_name+'_SMA_daily_returns'] = portfolio[stock2_name+'_SMA_crossover'].pct_change()
