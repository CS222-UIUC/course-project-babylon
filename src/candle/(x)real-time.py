...
def get_data_bars(symbols, rate, slow, fast):
    ...
    logging.warning(
        "{} logging started".format(datetime.datetime.now().strftime("%x %X"))
    )
    try:
        ...
        for x in symbols:
            ...
            data.loc[:, (x, "fast_ema_1min")] = (
                data[x]["close"].rolling(window=fast).mean()
            )
            data.loc[:, (x, "slow_ema_20min")] = (
                data[x]["close"].rolling(window=slow).mean()
            )
            data.loc[:, (x, "return_1_min")] = (
                data[x]["close"] - data[x]["close"].shift(1)
            ) / (data[x]["close"].shift(1))
            data.loc[:, (x, "diff")] = (
                data[x]["slow_ema_20min"] - data[x]["fast_ema_1min"]
            )
            ...
            data.loc[:, (x, "profit_change")] = (
                data[x]["close"] - data[x]["entry_price"]
            ) / (data[x]["entry_price"])
            data.loc[:, (x, "PL")] = (data[x]["close"] - data[x]["entry_price"]) * (
                data[x]["qty"]
            )
...
def run_checker(stocklist):
    ...
    while True:
        ...
        if (
            datetime.datetime.now(tz).weekday() >= 0
            and datetime.datetime.now(tz).weekday() <= 4
        ):
            # Checks market is open
            print("Trading in process " + datetime.datetime.now().strftime("%x %X"))
            if datetime.datetime.now(tz).time() > datetime.time(
                8, 30
            ) and datetime.datetime.now(tz).time() <= datetime.time(15, 00):
                ...
                for signal in signals:
                    if signals[signal] > 0:
                        ...
                        try:
                            api.submit_order(
                                signal, signals[signal], "buy", "market", "day"
                            )
                            print(
                                "{} bought {}  {} shares, portfolio value {} 🤑💵".format(
                            ...
                            logging.warning(
                                "{} Insufficient buying power".format(
                                    datetime.datetime.now(tz).strftime("%x %X")
                                )
                            )
                            print(
                        ...
                        try:
                            api.submit_order(
                                signal, -signals[signal], "sell", "market", "day"
                            )
                            print(
                                "{} sold   {} {} shares, portfolio value {} 🤖💵".format(
                                    datetime.datetime.now(tz).strftime("%x %X"),
                                    signal,
                                    signals[signal],
                                    api.get_account().equity,
                ...
                print(
                    "Sleeping",
                    round(time_to_open(datetime.datetime.now(tz)) / 60 / 60, 2),
                    "hours",
                )
